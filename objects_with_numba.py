...
@njit(fastmath=True)
def advection(size, cell_pos, cell_vel_prev, params, dt):
    come_from = cell_pos - (cell_vel_prev * dt)
...
class Fluid:
    ...
    def update(self, dt):
        ...
        for c in self.cells:
            ...
            cell_vel_prev = np.array(tuple(c.vel_prev))
            c.density_prev = float(advection(self.size, cell_pos, cell_vel_prev, dens_prev, dt))
        ...
        for c in self.cells:
            cell_pos = np.array(tuple(c.pos))
            cell_vel_prev = np.array(tuple(c.vel_prev))
            new_vel_x = advection(self.size, cell_pos, cell_vel_prev, prev_x_sep, dt)
            new_vel_y = advection(self.size, cell_pos, cell_vel_prev, prev_y_sep, dt)
            c.vel_prev = vec(float(new_vel_x), float(new_vel_y))
        ...
        for i in range(len(dens)):
            ...
            self.cells[i].density_prev = self.cells[i].density
    ...
    def clear_divergence(self):
        ...
        for x in range(self.size):
            for y in range(self.size):
                ...
                diff_x = (self.cells[index(self.size, x + 1, y)].vel_prev.x - self.cells[
                    index(self.size, x - 1, y)].vel_prev.x)
                diff_y = (self.cells[index(self.size, x, y + 1)].vel_prev.y - self.cells[
                    index(self.size, x, y - 1)].vel_prev.y)
        ...
        for x in range(self.size):
            for y in range(self.size):
                ...
                self.cells[index(self.size, x, y)].vel_prev -= vec(x_val, y_val)
